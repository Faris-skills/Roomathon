...
import ComparisonResult from "../components/ComparisionResult";
...
export default function ComparisionPage() {
  ...
  const handleReferenceChange = (file, url) => {
    setReferenceFile(file);
    setReferenceUrl(url);
  };

  const handleComparisonChange = (file, url) => {
        ...
        <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-6 md:p-10 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* <ImageUploader
              ...
              preview={referenceFile && URL.createObjectURL(referenceFile)}
              onImageChange={handleReferenceChange}
            /> */}
            <ImageUploader
  label="Upload Reference Image"
  preview={referenceFile}
...
/>
            ...
            {/* <ImageUploader
              ...
              preview={comparisonFile && URL.createObjectURL(comparisonFile)}
              onImageChange={handleComparisonChange}
            /> */}
            <ImageUploader
  label="Upload Comparison Image"
  preview={comparisonFile}
  onImageChange={setComparisonFile}
  roomId="room-123"
  imageType="comparision"
/>
        ...
        <div className="w-full max-w-4xl">
          <ComparisonResult
            results={results}
            referenceImage={referenceFile && URL.createObjectURL(referenceFile)}
            comparisonImage={
              comparisonFile && URL.createObjectURL(comparisonFile)
            }
